...
当たり判定（`hs.axuielement.systemElementAtPosition`）だけでは、つかめるかどうかは決められません。実測では**ウィンドウと同じ矩形を返しながらつかめない点**（IntelliJ）も、**別の矩形を返しながらつかめる点**（Excel）もありました。

そこで SpaceSaver は候補点を押したあと **12px 動かし、ウィンドウが追随したかを見ます**。追随すればそのまま掴んだ状態で Space の移動へ進み、動かなければマウスを放して次の候補へ移ります。位置が変わらず大きさだけ変わったときはリサイズ枠なので、これも失敗として扱います。つかめた位置は bundleID ごとに覚え、次のウィンドウから最初に試します。

当たり判定は 2 つの用途に絞って使います。**候補点が本当にそのウィンドウのものか**（`AXWindow` の祖先まで辿ってウィンドウ ID で照合）と、**押してはいけない要素でないか**（ボタン、タブ、プロキシアイコンなど）の確認です。
...
つかめない位置をつかむと、Space だけが切り替わってウィンドウが取り残されます。そうした場合はコンソールに次のように出て、同じアプリで 3 回続けて失敗するとその起動中は試行をやめ、位置とサイズの復元だけ行います。
