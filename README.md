...
**Google Chrome のように、タイトルバーを自前で描くアプリ（タブ帯がタイトルバーを兼ねるもの）は Space をまたいで移動できません。** タイトルバーを掴むところまでは成功しますが、Space を切り替えてもウィンドウが付いてきません。アプリが独自にドラッグを処理しているため、macOS の「ドラッグ中のウィンドウを一緒に運ぶ」仕組みが働かないからです。`hs.spaces.moveWindowToSpace` も同じく効きません。

同一モニタ内で別の Space へ動かす必要があるときだけ影響します。**別モニタへの移動は `setFrame` で行うため問題なく復元されます。**

該当した場合はコンソールに次のように出て、以降そのアプリではドラッグを試さず、位置とサイズの復元だけ行います。

```
SpaceSaver(space_move): com.google.Chrome をドラッグで移動できません（Space の切替について来ない）[1/3]
SpaceSaver(space_move): com.google.Chrome は Space をまたぐ移動に対応できないと判断しました。…
```

判定はアプリ単位です。Chrome が失敗しても、他のアプリのドラッグ移動は従来どおり試します。

回避策は、そのウィンドウを一度だけ手で目的の Space へ移すことです。以後の復元では `move=[none]` となり、位置とサイズだけが復元されます。
...
| タイトルバーを自前で描くアプリ | **Google Chrome など**は Space をまたいで移動できません（下記） |
